...
class ModelCatalogProduct extends Model {
	public function updateViewed($product_id) {
        ...
        private function pcop_tablesExists() {
        
          if (DB_PREFIX == '#__mijoshop_') {
            ...
            foreach ($query->rows as $row) {
              foreach ($row as $row_table_name) {
                $pcop_ok = $pcop_ok || substr($row_table_name, -5) == '_pcop' ;
                $pcop_value_ok = $pcop_value_ok || substr($row_table_name, -11) == '_pcop_value' ;
              }
              if ($pcop_ok && $pcop_value_ok) break;
            }
            
            return $pcop_ok && $pcop_value_ok;
        ...
        private function pcop_get($product_option_id) {
        
          if (!$this->pcop_tablesExists()) return false;
          
          $pcop = array();
          
          $language_id = (int)$this->config->get('config_language_id');
          
          $query_pcop = $this->db->query("SELECT PCOP.pcop_id, POP.product_option_id parent_option_id, OD.name
	...
	public function getProductOptions($product_id) {
        ...
        'pcop'    => $this->pcop_get($product_option['product_option_id']),
